...
namespace AccountingSystem.Pages.Reports
{
    ...
    public class IncomeStatementModel : PageModel
    {
        ...
        public IncomeStatementModel(IFinancialReportService reports, ApplicationDbContext db, UserManager<IdentityUser> userManager)
        {
            _reports = reports;
            _db = db;
        ...
        public decimal NetIncome => Rows.Sum(x => x.Amount);

        public async Task OnGetAsync()
        {
            Rows = await _reports.GetIncomeStatementAsync(From, To);
        }

        public async Task<IActionResult> OnPostExportCsvAsync(DateTime? from, DateTime? to)
        {
            var rows = await _reports.GetIncomeStatementAsync(from, to);
            ...
            return File(bytes, "text/csv", "IncomeStatement.csv");
        }
        ...
        public async Task<IActionResult> OnPostEmailAsync(DateTime? from, DateTime? to)
        {
            _db.SentEmails.Add(new SentEmail
            {
                ...
                Subject = "Income Statement Report",
                BodyHtml = "Income Statement generated.",
            ...
            StatusMessage = "Income Statement emailed to outbox.";
